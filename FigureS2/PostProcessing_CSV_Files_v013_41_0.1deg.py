...
CSVFolderName =r'F:\\15_Viscoelastic_Metamaterial\AbaqusNew\Model_v013\batch_v13_41_0.1deg_Visco-Agilus_norelax19mm\OutputOmega'
...
t_loading_MAT = 3*[1.5e-1,1.5e0,1.5e1,1.5e2,1.5e3,1.5e4]		+3*[1.5e5]		+3*[1.5*2.15e2,1.5*4.64e2,1.5*2.15e3,1.5*4.64e3] 
E_TriH_vec = 6*[1.]+6*[2.]+6*[0.5]		+[1.,2.,0.5]		+4*[1.]+4*[2.]+4*[0.5]	
...
eps_mat = CSVInput_Mat[:,2,:]
epsdot_mat,eps_mat2 = np.meshgrid(epsdot_loading_vec[MaterialIDS],CSVInput_Mat[:,2,0])

Omega_absavgmat = np.absolute(CSVInput_Mat[:,5,:])
...
ax.yaxis.set_label_coords(-0.29, 0.5)
ax.contourf(eps_mat,epsdot_mat,Omega_absavgmat,levels=np.linspace(-1,1, stepscmap),cmap=cmapOmega,extend='neither')
...
ax.xaxis.set_label_coords(0.5, -0.2)
ax.yaxis.set_label_coords(-0.29, 0.5)
ax.contourf(eps_mat,epsdot_mat,Omega_absmaxmat,levels=np.linspace(-1,1, stepscmap),cmap=cmapOmega,extend='neither')
...
axshapetest = [-0.5,0.28,0.62,0.62]
nb=103
fig=plt.figure(nb,figsize=(figsz[0]/3,figsz[1]) )
