...
for ind3 in MaterialIDS:
    ...
    RPT_U_Name = 'Output_U_v013_042-Mat-'+str(ind3)+'.rpt'
    U_Data_RAW = pd.read_csv(RPT_U_Name,sep='\s{3,}',skiprows=2)
    csv_XY_Name = 'Output_XYstart_v013_042.csv'
    ...
    Poisson_min = np.min(deps_x_comp,axis=0)/np.average(deps_loading_conMAT_comp,axis=0)
    Poisson_max = np.max(deps_x_comp,axis=0)/np.average(deps_loading_conMAT_comp,axis=0)
...
# =============================================================================
    ...
    ax.set_ylabel(r'$y_{n}$',rotation=0)
    ax.set_xlim([0,0.1])    
    ax.set_ylim([-0.5,0.5])
...
# =============================================================================
    ...
    if savefigures:
        ...
        plt.savefig(FigName+'.png',dpi=300,Transparent=True)       
    dicPoisson={}
    dicPoisson['Epsilon']=copy(eps_loading_vec[IndsCompression[1:]][:-30])
    dicPoisson['PoissonAverage']=copy(Poisson_avg[:-30])
    dicPoisson['PoissonMin']=copy(Poisson_min[:-30])
    dicPoisson['PoissonMax']=copy(Poisson_max[:-30])
    
    DFPoisson = pd.DataFrame(dicPoisson)
    DFPoisson.to_csv(FigName+'.csv',sep=',')
