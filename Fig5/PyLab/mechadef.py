...
"definitions of functions used in the other mechanical metamaterials programs"
...
def posadj(aperio, cellpos,fileout_gen='lattices/adjacency/gen/pentagone',fileout_pos='lattices/positions/gen/pentagone'):
    #Builds lattice out of unit cell geometry and combinatorial choice
    #input: 
    # aperio: combinatorial cell orientation (np array n x n, where n is system size)
    # cellpos: position of point within a unit cell (np array k*1, k is number of point in a cell, = 8 )
    #output: save lattice points and adjacency matrix
	...
	positions = np.zeros(shape = (posdim, 2))
	...
	np.savetxt(fileout_pos, positions, fmt='%d', delimiter=',')
	...
	np.savetxt(fileout_gen, adjacency, fmt='%d', delimiter=',')
